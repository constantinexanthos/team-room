...
TOOLS = {
    "team_room_ask": {
        "description": (
            ...
            "READING vs RENDERING — two separate jobs.\n\n"
            "READING (your job, mandatory): take in the full `messages` and "
            "`final_brief`. The deliberation is two AI minds reporting to you, "
            "the orchestrating agent. Their reasoning, their dissent, their "
            "lens-asymmetries, the things they considered and rejected — all of "
            "it is context for YOUR follow-up thinking, future decisions, and "
            "synthesis with the broader conversation. You are NOT an echo of "
            "the joint_read; you are the senior who heard the report and now "
            "acts on it. If team-room's reasoning shifts your view, say so. If "
            "it surfaced a risk worth flagging, flag it.\n\n"
            "RENDERING (what the user sees, minimal): the headline, not the "
            "article. The user can expand the tool-call widget to read the "
            "transcript themselves.\n"
            "  - converged: QUOTE `final_brief.joint_read` verbatim — don't "
            "    paraphrase the joint_read text. One short lead-in is fine "
            "    (\"Team Room's read:\"). OK to add ONE sentence of synthesis "
            "    if the deliberation revealed something the joint_read elided "
            "    and it's load-bearing for the user's decision (e.g. \"Codex "
            "    flagged X mid-dialogue — worth knowing if you go that way.\") "
            "    Don't do this performatively.\n"
            "  - forked: present `final_brief.fork` bullets concisely; ask the "
            "    user how they want to resolve if relevant.\n"
            "  - timed-out: one line (\"Team Room timed out at turn N — partial "
            "    in the expanded view\"). If the partial contained something "
            "    you'd act on, surface that sentence.\n"
            "  - failed: one line with `final_brief.error`.\n\n"
            "What NOT to render: do not paste `messages`, do not narrate the "
            "deliberation turn by turn, do not summarize each agent's position, "
            "do not include `final_brief` metadata or `session` info in your "
            "text response. The deliberation can be deep; the surfaced answer "
            "should be tight enough to fit in a single line of chat."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
